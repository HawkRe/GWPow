      ...
      for(int k = lower[2]; k <= upper[2]; k++)
      {
        for(int j = lower[1]; j <= upper[1]; j++)
        {
          for(int i = lower[0]; i <= upper[0]; i++)
          {
            ...
            {
              tot_vol += weight_array(i, j, k) *  pw3( a(i, j, k));
              conformal_avg +=
                pw3(a(i, j, k)) * array(i, j, k)
                * weight_array(i, j, k);
            }
          }
        }
              ...
              #pragma omp critical
              {
                for(int var_id = 0; var_id < output_num; var_id++)
                {
                  conformal_avg[var_id] +=
                    pw3(a(i, j, k)) * arrays[var_id](i, j, k)
                    * weight_array(i, j, k);
                }
              }
            }
...
#pragma omp parallel for collapse(2) reduction(+:tot_vol, oscillon_rho)
      for(int k = lower[2]; k <= upper[2]; k++)
      {
        for(int j = lower[1]; j <= upper[1]; j++)
        {
          for(int i = lower[0]; i <= upper[0]; i++)
          {
            tot_vol += weight_array(i, j, k) * pw3(a_array(i, j, k) );
	    if(weight_array(i,j,k) > 0
	       && DIFFr_array(i, j, k) > oscillon_frac_threshold * DIFFr_avg)
            {
              {
                oscillon_rho +=
                  pw3(a_array(i, j, k) ) * DIFFr_array(i, j, k)
                  * weight_array(i, j, k);
              }
            }
          }
