...
ScalarSim::ScalarSim(
  ...
  std::string vis_filename_in):CosmoSim(
    ...
    dim_in, input_db_in, l_stream_in, simulation_type_in, vis_filename_in),
			       ...
			       oscillon_frac_threshold(cosmo_scalar_db->getIntegerWithDefault("oscillon_frac_threshold", 2))
{
  ...
  idx_t ln)
{
  ...
  {
    if(ln > 0) return false;
    return scalar_ic_set_scalar_gaussian_random_defrost(hierarchy, ln, bssnSim, scalarSim, input_db->getDatabase("Scalar"));
      ...
      for(int k = lower[2]; k <= upper[2]; k++)
      {
        for(int j = lower[1]; j <= upper[1]; j++)
        {
          for(int i = lower[0]; i <= upper[0]; i++)
          {
            ...
            if(f(i, j, k) / gradient_scale_factor / (double)(ln+1) > adaption_threshold )
            {
              tag(i, j, k) = 1;
              ++ntag;
            }
  ...
  const std::shared_ptr<hier::PatchHierarchy>& hierarchy)
{
  ...
  cosmo_statistic->output_oscillon_fraction(
    hierarchy,
    bssnSim, weight_idx, step,   oscillon_frac_threshold );
